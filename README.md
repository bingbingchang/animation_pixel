...
**白衣长发角色当前采用的轻量路线（2026-09-21）：免费模型现成动作 → 白模序列帧 → 依据角色参考绘画 → 快速预览 → 满意后导出。** 见 [白模到角色序列帧技术路线](docs/13_white_model_sprite_character_workflow.md)。优先复用动作和检查明显问题，不要求视频生成、完整服装 3D 建模或逐帧精修。下述三连斩规范与历史案例保留各自适用范围。
